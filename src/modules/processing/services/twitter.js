...
export default async function(obj) {
    ...
    if (obj.id) {
        ...
        let TweetResultByRestId = await fetch(query, { headers: _headers, dispatcher }).then((r) => { return r.status === 200 ? r.json() : false }).catch((e) => { return false });
    ...
    if (obj.spaceId) {
        ...
        let AudioSpaceById = await fetch(query, { headers: _headers, dispatcher }).then((r) => {return r.status === 200 ? r.json() : false}).catch((e) => { return false });
        ...
        let streamStatus = await fetch(
            `https://twitter.com/i/api/1.1/live_video_stream/status/${AudioSpaceById.data.audioSpace.metadata.media_key}`,
            { headers: _headers, dispatcher }
        ).then((r) =>{ return r.status === 200 ? r.json() : false }).catch(() => { return false });
