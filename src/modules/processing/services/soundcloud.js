...
export default async function(obj) {
    ...
    let clientId = await findClientID(dispatcher);
    if (!clientId) return { error: 'ErrorSoundCloudNoClientId' };

    let link;
    if (obj.shortLink && !obj.author && !obj.song) {
        link = await fetch(`https://on.soundcloud.com/${obj.shortLink}/`, { redirect: "manual", dispatcher }).then((r) => {
    ...
    let json = await fetch(`https://api-v2.soundcloud.com/resolve?url=${link}&client_id=${clientId}`, { dispatcher }).then((r) => {
